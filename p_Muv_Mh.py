...
from uvlf import ms_mh_flattening, SFMS, Muv_Luv
import argparse
import hmf
...
def interp_kuv(SFR, Mstar, z, bounds_error=True, fill_value=None,
               interpolation_table_loc='/groups/astro/ivannik/projects/UVLF_clust/interpolation_table.npy'):
    """
...
if __name__ == "__main__":
    ...
    g = pUV_calc_numba(
        ...
        hmf_loc_10.dndlog10m * cosmo.h**3 * np.exp(- 5e8 / (hmf_loc_10.m / cosmo.h) ),
    ...
    #Example 1: Plotting p(Muv|Mh) for Mh = 1e10 and 1e11
    p10 = []
    for index in range(150):
        mcho, p0 = zip(*sorted(zip(g[1], g[0][:, index])))
        p0ac = np.interp(10, mcho, p0)
        p10.append(p0ac)
    p11 = []
    for index in range(150):
        mcho, p0 = zip(*sorted(zip(g[1], g[0][:, index])))
        p0ac = np.interp(11, mcho, p0)
        p11.append(p0ac)
    plt.plot(muv_bins, p10)
    plt.plot(muv_bins, p11, '--')
    plt.xlim(-24, -12)